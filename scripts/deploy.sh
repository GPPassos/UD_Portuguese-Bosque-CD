...
if cat documents/CF0001.conllu| python UniversalDependencies/tools/validate.py --lang pt 2> automatic-results/validate.log ; then
    echo -e "\n\`UniversalDependencies/tools/validate.py\`: SUCCESSFUL" >> automatic-results/README ;
else
    echo -e "\n\`UniversalDependencies/tools/validate.py\`: FAILED -- see \`validate.log\`" >> automatic-results/README ;
fi
